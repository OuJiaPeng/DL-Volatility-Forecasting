# [LEGACY] Volatility Forecasting with PatchTST

> **This is the archived BTC/PatchTST project (2025).** It has been superseded by the
> `volforecast` package at the repo root (SPX realized-vol term structure, leakage-safe
> by construction). Kept for reproducibility and the audit trail. All scripts here use
> CWD-relative paths — run them **from inside `legacy/`** (`cd legacy && make train`).
...
> **⚠️ The figures below are provisional and pending regeneration.** A 2026 code audit
> found that the **rolling** GARCH/Kalman baselines were mis-aligned by 60 trading days
> against the actuals, so the two `*(Rolling)` rows are unreliable (the PatchTST-vs-**Full**
> headline is unaffected). That alignment bug and a silently-broken ensemble driver are now
> fixed in code, and **HAR-RV / Persistence (RW) / EWMA** baselines have been added — but the
> table and plot have not yet been re-run. A known caveat remains (see code `TODO(audit #2)`):
> PatchTST's input is built from the forward-looking target, giving it a look-ahead the
> classical baselines don't have. Re-run `make targets && make ensemble && make plot` on
> fresh data to regenerate.
